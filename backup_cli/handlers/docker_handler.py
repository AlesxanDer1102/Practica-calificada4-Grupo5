...
class DockerHandler:
    ...
    def _is_postgres_container(self, container_data: Dict, name_filter: str = None) -> bool:
    ...
    def select_container_interactive(self, name_filter: str = None) -> Optional[str]:
        ...
        if len(containers) == 1:
            ...
            print(f"Contenedor de PostgreSQL detectado automáticamente: {container_name}")
        ...
        while True:
            try:
                selection = input("Seleccione el número del contenedor (0 para cancelar): ").strip()
    ...
    def check_container_status(self, container_name: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error al verificar estado del contenedor {container_name}: {e}")
            return False

    def execute_command(self, container_name: str, command: List[str],
