...
class BackupNameValidator:
    ...
    def validate_backup_name(cls, name: str) -> tuple[bool, str]:
        ...
        if re.search(cls.INVALID_CHARS_PATTERN, name):
            return (
                False,
                f"El nombre contiene caracteres inválidos: {cls.INVALID_CHARS_PATTERN}",
            )
        ...
        if len(name) > cls.MAX_NAME_LENGTH:
            return (
                False,
                f"El nombre es muy largo (máximo {cls.MAX_NAME_LENGTH} caracteres)",
            )
    ...
    @classmethod
    def resolve_backup_filename(
        cls, backup_dir: Path, custom_name: str = None, force_overwrite: bool = False
    ) -> tuple[str, bool]:
        """
        ...
        if custom_name:
            ...
            if backup_path.exists() and not force_overwrite:
                ...
                return (
                    backup_filename,
                    False,
                )  # False indica que se usó el nombre original
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
