...
class TestDockerConnection:
    ...
    def test_check_docker_container_exists(
        self, orchestrator_instance, mock_docker_container
    ):
        """
        ...
        mock_docker_container.assert_called_once_with(
            ["docker", "inspect", "test_db"], capture_output=True, text=True, timeout=10
        )

    def test_check_docker_container_not_found(
        self, orchestrator_instance, mock_docker_container_not_found
    ):
        """
