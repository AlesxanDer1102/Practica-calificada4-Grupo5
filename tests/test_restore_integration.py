...
"""
    ...
    def test_list_backups_with_multiple_files(self, orchestrator, temp_backup_dir, sample_backup_content):
        """Test que verifica el listado de múltiples backups"""
        # Crear archivos de backup de prueba
        backup1 = self.create_sample_backup(temp_backup_dir, "backup_2024_01_01.sql", sample_backup_content)
        backup2 = self.create_sample_backup(temp_backup_dir, "backup_2024_01_02.sql", sample_backup_content)
    ...
    def test_validate_backup_integrity_valid_file(self, orchestrator, temp_backup_dir, sample_backup_content):
        """Test que verifica validación de backup válido"""
        backup_path = self.create_sample_backup(temp_backup_dir, "valid_backup.sql", sample_backup_content)
        
        result = orchestrator.validate_backup_integrity(backup_path)
        assert result is True

    def test_validate_backup_integrity_nonexistent_file(self, orchestrator, temp_backup_dir):
    ...
    def test_validate_backup_integrity_invalid_content(self, orchestrator, temp_backup_dir):
        """Test que verifica validación de contenido inválido"""
        invalid_content = "This is not a SQL backup file"
        invalid_backup = self.create_sample_backup(temp_backup_dir, "invalid.sql", invalid_content)
        
        result = orchestrator.validate_backup_integrity(invalid_backup)
        assert result is False

    @patch('builtins.input')
    def test_select_backup_interactive_valid_selection(self, mock_input, orchestrator, temp_backup_dir, sample_backup_content):
        """Test de selección interactiva con selección válida"""
        # Crear backup de prueba
        backup_path = self.create_sample_backup(temp_backup_dir, "test_backup.sql", sample_backup_content)
    ...
    @patch('builtins.input')
    def test_select_backup_interactive_cancel(self, mock_input, orchestrator, temp_backup_dir, sample_backup_content):
        """Test de selección interactiva con cancelación"""
        # Crear backup de prueba
        self.create_sample_backup(temp_backup_dir, "test_backup.sql", sample_backup_content)
    ...
    @patch('builtins.input')
    def test_confirm_restore_operation_yes(self, mock_input, orchestrator, temp_backup_dir, sample_backup_content):
    ...
    @patch('builtins.input')
    def test_confirm_restore_operation_no(self, mock_input, orchestrator, temp_backup_dir, sample_backup_content):
    ...
    @patch('builtins.input')
    def test_restore_database_successful(self, mock_input, mock_subprocess, orchestrator, temp_backup_dir, sample_backup_content):
    ...
    @patch('subprocess.run')
    @patch('builtins.input')
    def test_restore_database_psql_failure(self, mock_input, mock_subprocess, orchestrator, temp_backup_dir, sample_backup_content):
        ...
        # Configurar mocks
        mock_input.return_value = "si"  # Confirmar restauración
        mock_subprocess.return_value = Mock(returncode=1, stderr="Database connection failed")
    ...
    @patch('builtins.input')
    def test_restore_database_container_not_found(self, mock_input, orchestrator, temp_backup_dir, sample_backup_content):
    ...
    @patch('builtins.input')
    def test_restore_database_user_cancellation(self, mock_input, orchestrator, temp_backup_dir, sample_backup_content):
    ...
    def test_restore_database_invalid_backup(self, orchestrator, temp_backup_dir):
        ...
        invalid_backup = self.create_sample_backup(temp_backup_dir, "invalid.sql", "invalid content")
    ...
    @patch('subprocess.run')
    @patch('builtins.input')
    def test_restore_database_timeout(self, mock_input, mock_subprocess, orchestrator, temp_backup_dir, sample_backup_content):
        ...
        # Configurar mocks
        mock_input.return_value = "si"  # Confirmar restauración
        mock_subprocess.side_effect = subprocess.TimeoutExpired(cmd=["psql"], timeout=300)
    ...
    @patch('builtins.input')
    def test_full_restore_workflow(self, mock_input, orchestrator, temp_backup_dir, sample_backup_content):
        """Test del flujo completo de restauración desde selección interactiva"""
        # Crear múltiples backups
        backup1 = self.create_sample_backup(temp_backup_dir, "backup1.sql", sample_backup_content)
        backup2 = self.create_sample_backup(temp_backup_dir, "backup2.sql", sample_backup_content)
