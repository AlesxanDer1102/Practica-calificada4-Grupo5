...
class TestK8sIntegration:
    ...
    def sample_test_data(self):
        ...
        return {
            ...
            'productos': [
                {'producto_id': 1, 'nombre_producto': 'Laptop HP', 'manufacturador': 'HP', 'precio': 850.00},
                {'producto_id': 2, 'nombre_producto': 'Mouse Logitech', 'manufacturador': 'Logitech', 'precio': 25.00}
            ],
            'pedidos': [
...
"""
    ...
    def test_k8s_data_integrity_verification(self, mock_run, k8s_orchestrator, sample_test_data):
        ...
        for table, expected_count in zip(tables, expected_counts):
            result = k8s_orchestrator.handler.execute_command(
                "postgres-0", 
                ["psql", "-U", "postgres", "-d", "pc_db", "-t", "-c", f"SELECT COUNT(*) FROM {table};"]
            )
            
            assert mock_run.called

    @patch("subprocess.run")
    def test_k8s_full_integration_workflow(self, mock_run, k8s_orchestrator, temp_backup_dir, sample_test_data):
        ...
        with patch("builtins.open", create=True):
            with patch("pathlib.Path.stat") as mock_stat:
                ...
                backup_result = k8s_orchestrator.create_backup(custom_name="integration_test")
    ...
    def test_k8s_error_handling_pod_not_found(self, temp_backup_dir):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(returncode=1, stderr="pod 'nonexistent-pod' not found")
