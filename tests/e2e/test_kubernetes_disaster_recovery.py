...
class TestKubernetesDisasterRecovery:
    ...
    def kubernetes_environment(self):
        ...
        try:
            result = subprocess.run(["kubectl", "version", "--client"], capture_output=True, timeout=10)
        ...
        try:
            result = subprocess.run(["kubectl", "get", "nodes"], capture_output=True, timeout=15)
            if result.returncode != 0:
        ...
        # Cleanup previo
        subprocess.run(["kubectl", "delete", "pod", pod_name, "-n", namespace], capture_output=True)
        time.sleep(5)
...
"""
        ...
        # Aplicar manifest
        result = subprocess.run(["kubectl", "apply", "-f", manifest_file], capture_output=True, timeout=30)
        if result.returncode != 0:
            pytest.skip(f"No se pudo crear pod: {result.stderr}")

        # Esperar que esté corriendo
        for _ in range(30):  # 30 intentos x 10s = 5min max
    ...
    def test_kubernetes_pod_resilience(self, kubernetes_environment):
        ...
        assert "pvc" in disaster_result["disaster_type"] or "volume" in disaster_result["disaster_type"]
    ...
    def test_kubernetes_namespace_isolation(self, kubernetes_environment):
        ...
        assert namespace in result.get("target", pod_name) or result.get("target") == pod_name
